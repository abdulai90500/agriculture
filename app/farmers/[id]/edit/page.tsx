...
import { updateFarmer, getCrops, getLivestock, getFarmerById } from "../../actions";

export default function EditFarmer() {
  const router = useRouter();
  const params = useParams();
  const farmerId = typeof params?.id === 'string' ? params.id : Array.isArray(params?.id) ? params.id[0] : '';
  ...
  useEffect(() => {
    // Fetch available crops, livestock and the farmer data simultaneously
    Promise.all([
      getCrops(),
      getLivestock(),
      getFarmerById(farmerId)
    ]).then(([cropsList, livestockList, farmerInfo]) => {
      setAvailableCrops(cropsList);
      setAvailableLivestock(livestockList);
      ...
      if (farmerInfo) {
        setFormData({
          ...
          status: farmerInfo.status || "active"
        });
        
        if (farmerInfo.crops) {
          setSelectedCrops(farmerInfo.crops.map((c: any) => ({
            cropId: c.cropId,
            area: c.area
          })));
        }
        
        if (farmerInfo.livestock) {
          setSelectedLivestock(farmerInfo.livestock.map((l: any) => ({
            livestockId: l.livestockId,
            count: l.count
          })));
        }
      } else {
        setMessage({ type: "error", text: "Farmer not found" });
      }
      setFetching(false);
    });
  }, [farmerId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage({
          type: "error",
          text: result.error || "Failed to update farmer"
        });
      }
    } catch (error) {
      setMessage({
        type: "error",
        text: "Error updating farmer"
      });
    } finally {
      setLoading(false);
    }
  ...
  if (fetching) {
    return (
      <div className="flex">
        <Sidebar />
        <div className="flex-1 p-6 bg-gray-50 min-h-screen ml-64 flex items-center justify-center">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-2xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <select
              ...
              className={`p-2 border rounded-lg font-semibold ${formData.status.toLowerCase() === 'active' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200'}`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <button
                type="submit"
                disabled={loading || message?.text === "Farmer not found"}
